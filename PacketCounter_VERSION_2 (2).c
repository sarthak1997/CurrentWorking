...
int main()
{
    ...
    if(k==1 || k==2 || k==3 || k==4)
    {
        printf("\nEnter number of unique ip - ");
        scanf("%d",&unique_ip);
        printf("\nEnter total ip's to generate - ");
        scanf("%d",&iteration);
        //i/p required only when test case 4 is checked
        if(k!=4)
        {
            printf("\nEnter sleep per iteration - ");
            scanf("%d",&sleep_per_iteration);
        }
        printf("\nEnter ip's to generate before sleep - ");
        scanf("%d",&iteration_size);
    }
    //inputs taken
